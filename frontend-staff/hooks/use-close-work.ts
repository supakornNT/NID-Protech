...
export function useCloseWork() {
  ...
  const fetchRequests = useCallback(async (status: "pending" | "history" = "pending") => {
    setLoading(true);
    setError(false);

    try {
      const response = await fetch(`${API_BASE_URL}/admin/close-work/requests?status=${status}`);
      if (!response.ok) throw new Error("Failed to fetch close-work requests");
  ...
  const fetchRequestEvidence = useCallback(async (requestId: number): Promise<CloseAttachment[]> => {
    try {
      const response = await fetch(
  ...
  const fetchLatestReject = useCallback(async (requestId: number): Promise<string | null> => {
    try {
      const response = await fetch(
  ...
  const fetchTicketAttachments = useCallback(async (ticketId: number): Promise<CloseAttachment[]> => {
    try {
      const response = await fetch(
  ...
  const rejectTicket = useCallback(
    async (
      ...
      const response = await fetch(`${API_BASE_URL}/admin/close-work/tickets/${ticketId}/reject`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const changeRequestState = useCallback(
    async (requestId: number, status: string, note: string, changedById: number) => {
      const response = await fetch(`${API_BASE_URL}/admin/close-work/requests/${requestId}/change-state`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
